...
module "infra_foundation" {
  ...
  vpc_public_egress_traffic_using_nat_gw_or_fck_nat = "nat"

  ecs_cluster_name                                    = "ecs-${local.environment}"
  ecs_fargate_default_capacity_provider_strategy      = 50
  ecs_fargate_spot_default_capacity_provider_strategy = 50
  ecs_cluster_tags = {
    Terraform   = "true"
    Environment = local.environment
    Context     = "ecs"
